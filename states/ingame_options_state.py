# the script for the ingame options screen
# accessed via the pause menu only
        ...
        for event in events:
            if event.type == pygame.QUIT:
                ...
                if self.back_button.collidepoint(event.pos):
                    print("changed back to the ingame state")
                    self.game.state_manager.change_state("game")
                if self.fullscreen_button.collidepoint(event.pos):
                    print("Fullscreen Active. Sorry The Fullscreen Doesn't Work.")
                if self.windowed_button.collidepoint(event.pos):
                    print("Windowed Active. Sorry The Windowed Mode Doesn't Work.")
